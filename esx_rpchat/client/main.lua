...
AddEventHandler('esx_rpchat:sendMe', function(playerId, title, message, color)
	...
	if targetPed == source or #(sourceCoords - targetCoords) < 20 then
      ...
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 16px;"><i class=""></i> /ME │ {0} -> {1}</div>',
        args = { title, message }
    })
	end
  end
...
AddEventHandler('esx_rpchat:sendDo', function(playerId, title, message, color)
	...
	if targetPed == source or #(sourceCoords - targetCoords) < 20 then
		TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(220, 173, 17, 0.6); border-radius: 16px;"><i class=""></i> /DO │ {0} -> {1}</div>',
        args = { title, message }
    })
	end
  end
...
AddEventHandler('esx_rpchat:sendStav', function(playerId, title, message, color)
	...
	if targetPed == source or #(sourceCoords - targetCoords) < 20 then
		TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(220, 173, 17, 0.6); border-radius: 16px;"><i class=""></i> [STAV] │ {0} -> {1}</div>',
        args = { title, message }
    })
	end
  end
...
AddEventHandler('esx_rpchat:sendDo2', function(playerId, title, message, color)
	...
	if targetPed == source or #(sourceCoords - targetCoords) < 20 then
		TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(255, 0, 255, 0.6); border-radius: 16px;"><i class=""></i> /DOC │ {0} -> {1}</div>',
        args = { title, message }
    })
	end
  end
...
AddEventHandler('esx_rpchat:sendLocalOOC', function(playerId, title, message, color)
	...
	if targetPed == source or #(sourceCoords - targetCoords) < 20 then
		TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(173, 216, 230, 0.3); border-radius: 16px;"><i class=""></i> Local │ {0} -> {1}</div>',
        args = { title, message }
    })
	end
  end
