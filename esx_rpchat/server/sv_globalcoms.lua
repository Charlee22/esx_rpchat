...
RegisterCommand('sheriff', function(source, args, rawCommand)
    ...
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(180, 61, 34, 1); border-radius: 10px;"><i class="fas fa-star"></i> LASD: {0}</div>',
          args = {toSay}
      })
  else 
    TriggerClientEvent('chat:addMessage', source, {
...
RegisterCommand('stav', function(source, args, raw)
  if source == 0 then
    print('esx_rpchat: you can\'t use this command from rcon!')
    return
  end
  ...
  TriggerClientEvent('esx_rpchat:sendStav', -1, source, name, args, { 255, 255, 0 })
